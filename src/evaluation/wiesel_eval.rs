...
impl Default for WieselSettings {
    fn default() -> Self {
        Self {
            pawn_value: Default::default(),
            version: Default::default(),
            piece_weight: Default::default(),
        }
    }
}
...
pub fn static_piece_weight(
    ...
    for i in 0..5 {
        sum +=
            (whitepieces[i].count_ones() as i32 - blackpieces[i].count_ones() as i32) * weights[i];
    }
    return sum;
}

pub fn absolute_field_control(
    bb: &BitBoard,
    whitemoves: &MoveVector,
    blackmoves: &MoveVector,
    f: &dyn Fn(i32, &i8) -> i32,
) -> i32 {
