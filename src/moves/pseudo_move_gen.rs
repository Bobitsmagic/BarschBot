...
use crate::{
    ...
    game::{board_state::BoardState, game_flags::GameFlags},
    moves::slider_gen::{gen_bishop_moves, gen_rook_moves},
};
...
pub fn gen_pseudo_legal_moves_bitboard(board_state: &BoardState, flags: &GameFlags) -> MoveVector {
    ...
    //Captures
    let pawn_targets = opponent
        | (if flags.en_passant_square.is_valid_square() {
    ...
    for target_square in moveset.iterate_squares() {
        moves.push(ChessMove::new(
            king_square,
            target_square,
            PieceType::King.colored(moving_color),
            piece_board[target_square],
        ));
    }
    //Castling
