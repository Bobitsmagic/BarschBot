use crate::{
    board::{
        piece_type::{ColoredPieceType, PieceType::Pawn},
        square::{self, Square},
    },
    moves::move_gen::MoveVector,
};
...
impl ChessMove {
    ...
    pub fn san_move(&self, move_list: &MoveVector) -> String {
        ...
        let piece_name = match self.move_piece.piece_type() {
            Pawn => (if self.is_capture() {
                square::FILE_NAMES[self.start.file() as usize]
            } else {
                ""
            })
            .to_owned(),
        ...
        let promotion = if self.is_promotion() {
            self.promotion_piece.to_char().to_string()
        } else {
            "".to_owned()
        };
