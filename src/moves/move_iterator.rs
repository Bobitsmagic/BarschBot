...
use crate::board::{
    bit_array::BitArray, piece_board::PieceBoard, piece_type::PieceType, player_color::PlayerColor,
    rank, square::Square,
};
...
impl MoveIterator {
    ...
    pub fn iterate_uci_squares(&self, color: PlayerColor) -> impl Iterator<Item = (i8, i8)> + '_ {
        ...
        return self
            .squares
            .iter()
            .zip(self.bit_boards.iter())
            .flat_map(|(start_square, bit_board)| {
                bit_board
                    .iterate_squares()
                    .map(|target_square| (*start_square, target_square))
            })
            .chain(
                self.pawn_pushes
                    .iterate_squares()
                    .map(move |target_square| (target_square.translate(0, dy), target_square)),
            )
            .chain(
                self.double_pushes
                    .iterate_squares()
                    .map(move |target_square| (target_square.translate(0, 2 * dy), target_square)),
            )
            .chain(
                self.left_pawn_captures
                    .iterate_squares()
                    .map(move |target_square| (target_square.translate(1, dy), target_square)),
            )
            .chain(
                self.right_pawn_captures
                    .iterate_squares()
                    .map(move |target_square| (target_square.translate(-1, dy), target_square)),
            );
    }
    ...
    pub fn iterate_piece_squares(&self) -> impl Iterator<Item = (i8, i8)> + '_ {
        return self.squares.iter().zip(self.bit_boards.iter()).flat_map(
            |(start_square, bit_board)| {
                bit_board
                    .iterate_squares()
                    .map(|target_square| (*start_square, target_square))
            },
    ...
    pub fn iterate_piece_moves<'a>(
        &'a self,
        piece_board: &'a PieceBoard,
    ) -> impl Iterator<Item = ChessMove> + 'a {
        return self.squares.iter().zip(self.bit_boards.iter()).flat_map(
            move |(start_square, bit_board)| {
                ...
                if target_square.rank() == rank::R1 || target_square.rank() == rank::R8 {
                    return vec![
                        ...
                        ChessMove::new_pawn(
                            ...
                            pt,
                            cpt,
                            PieceType::Bishop.colored(color),
                        ),
                        ChessMove::new_pawn(
                            start_square,
                            target_square,
                            pt,
                            cpt,
                            PieceType::Knight.colored(color),
                        ),
                    ]
                    .into_iter();
                } else {
    ...
    pub fn iterate_all_moves<'a>(
        &'a self,
        piece_board: &'a PieceBoard,
        color: PlayerColor,
    ) -> impl Iterator<Item = ChessMove> + 'a {
        return self
            .iterate_piece_moves(piece_board)
            .chain(self.iterate_pawn_moves(piece_board, color));
    }

    pub fn iterate_square_moves<'a>(
        &'a self,
        piece_board: &'a PieceBoard,
        color: PlayerColor,
    ) -> impl Iterator<Item = ChessMove> + 'a {
        return self
            .iterate_uci_squares(color)
            .flat_map(move |(start_square, target_square)| {
                let pt = piece_board[start_square];
                let cpt = piece_board[target_square];
                if pt.is_pawn() {
                    if target_square.rank() == rank::R1 || target_square.rank() == rank::R8 {
                        ...
                        return vec![ChessMove::new(start_square, target_square, pt, cpt)]
                            .into_iter();
                    }
                } else {
                    return vec![ChessMove::new(start_square, target_square, pt, cpt)].into_iter();
